...
export class ProviderSearchComponent implements OnInit {
    ...
    profiles = [
    {prof:"Area Belconnen", name:"Belconnen"},
    {prof:"Area Woden",  name:"Woden"},
    {prof:"Area Kambah", name:"Kambah"}
    // {name:"Belconnen"},
    // {name:"Woden"},
    // {name:"Kambah"}
    ];
    ...
    profileSelect(event){
        var SelectedDrop = this.filter(event.name);
       console.log(SelectedDrop[0].lat, "selected dropdown************");

        this.setProviderInContext(SelectedDrop[0].lat, SelectedDrop[0].lng);

    //    if (event.name == 'Belconnen'){
    //     this.setProviderInContext(-35.239172, 149.062821);
    //    }else if(event.name == 'Kambah'){
    //     this.setProviderInContext(-35.396988, 149.069960);
    //    }else if(event.name == 'Woden'){
    //     this.setProviderInContext(-35.353014, 149.090680);
    //    }
    ...
    saveSearch(event){
        ...
        if (!this.selectedProvider) {
            alert("Please enter a valid Provider in Search Box");
        } else {

            if (confirm("Do you want to save Search in Local Memory?") == true) {
                // "You pressed OK!";
                var UserSave = prompt("Please enter your Search Name", "<Area_name>");
                // this.profiles.push(SearchStr:SearchProf, name:this.selectedProvider);
                //var providerEntry = this.filter(this.selectedProvider.name);
                var userprof = {prof: UserSave, name: this.selectedProvider.name};
                this.profiles.push(userprof);//this.selectedProvider);
            } else {
                //"You pressed Cancel!";
            }
        }
         console.log(this.selectedProvider,this.profiles,event, "dropdown************");
